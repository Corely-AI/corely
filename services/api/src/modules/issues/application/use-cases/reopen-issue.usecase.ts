...
import type { ReopenIssueRequest } from "@corely/contracts";
...
const ACTION_KEY = "issues.reopen";
...
export class ReopenIssueUseCase {
  ...
  async execute(input: ReopenIssueRequest, ctx: UseCaseContext): Promise<Issue> {
    assertCan(ctx);
    ...
    assertIssueStatusTransition(issue.status, "REOPENED");
