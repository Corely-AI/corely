...
export class TaxController {
  ...
  async getProfile(@Req() req: Request): Promise<GetTaxProfileOutput> {
    ...
    const profile = unwrap(await this.getTaxProfileUseCase.execute(undefined, ctx));
  ...
  async upsertProfile(@Body() body: unknown, @Req() req: Request): Promise<UpsertTaxProfileOutput> {
    ...
    const profile = unwrap(await this.upsertTaxProfileUseCase.execute(input, ctx));
  ...
  async listCodes(@Req() req: Request): Promise<ListTaxCodesOutput> {
    const ctx = buildTaxUseCaseContext(req);
    const codes = unwrap(await this.listTaxCodesUseCase.execute(undefined, ctx));
  ...
  async createCode(@Body() body: unknown, @Req() req: Request): Promise<CreateTaxCodeOutput> {
    ...
    const code = unwrap(await this.createTaxCodeUseCase.execute(input, ctx));
  ...
  async calculate(@Body() body: unknown, @Req() req: Request): Promise<CalculateTaxOutput> {
    ...
    const breakdown = unwrap(await this.calculateTaxUseCase.execute(input, ctx));
  ...
  async lockSnapshot(@Body() body: unknown, @Req() req: Request): Promise<LockTaxSnapshotOutput> {
    ...
    const snapshot = unwrap(await this.lockTaxSnapshotUseCase.execute(input, ctx));
  ...
  async getSummary(@Req() req: Request): Promise<GetTaxSummaryOutput> {
    const ctx = buildTaxUseCaseContext(req);
    return unwrap(await this.getTaxSummaryUseCase.execute(undefined, ctx));
  }
  ...
  async getReport(@Param("id") id: string, @Req() req: Request) {
    const ctx = buildTaxUseCaseContext(req);
    const report = unwrap(await this.getTaxReportUseCase.execute(id, ctx));
    return { report };
  }
  ...
  async listVatPeriods(@Query() query: any, @Req() req: Request) {
    ...
    return unwrap(await this.listVatPeriodsUseCase.execute(input, ctx));
  }
  ...
  async getVatPeriodDetails(@Param("key") key: string, @Req() req: Request) {
    const ctx = buildTaxUseCaseContext(req);
    return unwrap(await this.getVatPeriodDetailsUseCase.execute(key, ctx));
  }
  ...
  async markVatPeriodNil(@Param("key") key: string, @Body() body: unknown, @Req() req: Request) {
    ...
    const ctx = buildTaxUseCaseContext(req);
    return unwrap(await this.markVatPeriodNilUseCase.execute({ ...input, periodKey: key }, ctx));
  }
  ...
  async archiveVatPeriod(@Param("key") key: string, @Body() body: unknown, @Req() req: Request) {
    ...
    const ctx = buildTaxUseCaseContext(req);
    return unwrap(await this.archiveVatPeriodUseCase.execute({ ...input, periodKey: key }, ctx));
  }

  @Get("reports/vat/quarterly/:key/pdf-url")
  async getVatPeriodPdfUrl(@Param("key") key: string, @Req() req: Request) {
    const ctx = buildTaxUseCaseContext(req);
    return unwrap(await this.requestTaxReportPdfUseCase.execute({ periodKey: key }, ctx));
  }

  @Get("reports/:id/pdf-url")
  async getReportPdfUrl(@Param("id") id: string, @Req() req: Request) {
    const ctx = buildTaxUseCaseContext(req);
    return unwrap(await this.requestTaxReportPdfUseCase.execute({ reportId: id }, ctx));
  }

  @Get("consultant")
  async getConsultant(@Req() req: Request): Promise<GetTaxConsultantOutput> {
    const ctx = buildTaxUseCaseContext(req);
    return unwrap(await this.getTaxConsultantUseCase.execute(undefined, ctx));
  }
  ...
  async upsertConsultant(
    ...
    const ctx = buildTaxUseCaseContext(req);
    return unwrap(await this.upsertTaxConsultantUseCase.execute(input, ctx));
  }
