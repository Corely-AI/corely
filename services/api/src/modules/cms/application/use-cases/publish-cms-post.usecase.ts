...
type PublishCmsPostParams = { postId: string };

export class PublishCmsPostUseCase extends BaseUseCase<PublishCmsPostParams, { post: CmsPostDto }> {
 constructor(private readonly useCaseDeps: Deps) {
 super({ logger: useCaseDeps.logger });
 }

 protected async handle(
 input: PublishCmsPostParams,
 ctx: UseCaseContext
 ...
 try {
 post.publish(this.useCaseDeps.clock.now());
 } catch (error) {
