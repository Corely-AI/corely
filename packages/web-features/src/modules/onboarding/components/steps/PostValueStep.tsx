...
export const PostValueStep = ({ config, locale, onAdvance, isSaving }: StepComponentProps) => {
  ...
  const cta = config.ctaLabel?.[locale] || config.ctaLabel?.["en"] || "Go to dashboard";

  return (
    <div className="mx-auto flex max-w-2xl flex-col p-6 lg:p-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
        ...
        >
          <CardHeader className="p-5">
            ...
            <CardDescription>
              Give your employees access to add cash entries using the tablet app.
            </CardDescription>
        ...
        >
          <CardHeader className="p-5">
            ...
            <CardDescription>
              Set up custom expense and income categories for better reporting.
            </CardDescription>
            <div className="mt-4 text-sm font-medium text-amber-600 dark:text-amber-500 flex items-center gap-1">
              <Plus className="w-4 h-4" /> Edit categories
            </div>
          </CardHeader>
        </Card>
      </div>
