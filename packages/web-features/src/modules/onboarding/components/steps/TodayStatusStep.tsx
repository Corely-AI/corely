...
export const TodayStatusStep = ({ config, locale, onAdvance, isSaving }: StepComponentProps) => {
  ...
  const { progress } = useOnboarding({ config: config as any });
  ...
  return (
    ...
    >
      ...
      {receiptStepStatus === "skipped" ? (
        <div
          className="mb-6 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-900"
          data-testid="onboarding-receipt-reminder"
        >
          {t("onboarding.status.receiptMissing")}
        </div>
      ) : null}

      {workflowCopy ? (
        <div
          className="mb-8 rounded-xl border border-primary/20 bg-primary/5 p-4 text-sm text-foreground"
      ...
      <div className="p-4 bg-muted/40 rounded-xl border border-dashed border-border mb-10 text-sm flex gap-3">
        ...
        <div className="text-muted-foreground">
          <strong className="text-foreground">{t("onboarding.status.tipTitle")}</strong>{" "}
      ...
      <div className="flex justify-end">
        ...
        >
          <span>{t("onboarding.status.cta")}</span>
