...
export const envSchema = z.object({
  ...
  CORELY_PROXY_KEY_APP: z.string().min(1).optional(),
  CORELY_PROXY_KEY_POS: z.string().min(1).optional(),
  CORELY_PROXY_KEY_CRM: z.string().min(1).optional(),
...
export const SECRET_ENV_KEYS: ReadonlySet<keyof Env> = new Set([
  ...
  "CORELY_PROXY_KEY_APP",
  "CORELY_PROXY_KEY_POS",
  "CORELY_PROXY_KEY_CRM",
]);
...
export function validateEnv(env: NodeJS.ProcessEnv = process.env): Env {
  ...
  try {
    const validatedEnv = envSchema.parse(cleanedEnv);

    if (validatedEnv.NODE_ENV === "production") {
      const missingProxyKeys = [
        "CORELY_PROXY_KEY_APP",
        "CORELY_PROXY_KEY_POS",
        "CORELY_PROXY_KEY_CRM",
      ].filter((key) => !validatedEnv[key as keyof Env]);

      if (missingProxyKeys.length > 0) {
        console.warn(
          `[config] Missing trusted surface proxy keys: ${missingProxyKeys.join(
            ", "
          )}. Requests without trusted proxy keys will fall back to the platform surface.`
        );
      }
    }
