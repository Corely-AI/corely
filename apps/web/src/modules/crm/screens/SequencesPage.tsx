  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        <h1 className="text-h1 text-foreground">Sequences</h1>
        <Button variant="accent" onClick={() => navigate("/crm/sequences/new")} disabled>
          <Plus className="h-4 w-4" />
          New Sequence (Coming Soon)
        </Button>
      </div>
      ...
      {sequenceList.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sequenceList.map((seq) => (
            <Card key={seq.id} className="cursor-pointer hover:border-primary/50 transition-colors" onClick={() => {}}>
