...
export function RentalDetailPage() {
  ...
  const checkAvailabilityEnabled = Boolean(property && dateRange.from && dateRange.to);

  const { data: availability, isFetching: isCheckingAvailability } = useQuery({
    queryKey: ["rentals", "availability", slug, workspaceSlug, fromDate, toDate],
    queryFn: () =>
      publicApi.checkRentalAvailability({
        propertySlug: slug!,
        from: fromDate,
        to: toDate,
        workspaceSlug,
  ...
  const allImages = useMemo(() => {
    ...
    return [...property.images].sort((a, b) => a.sortOrder - b.sortOrder);
  }, [property]);
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl font-bold">Missing rental</h1>
          <p className="text-muted-foreground max-w-md">
            Please select a property from the rentals list.
          </p>
        </div>
